...
test.describe('Main Application Flow', () => {
  ...
  test('should verify all library demos load without crashing', async ({
    page,
  }) => {
    for (const lib of LIBRARIES) {
      await page.goto(
        `http://localhost:8080/?benchmark=true&library=${encodeURIComponent(lib)}&nodes=10`,
      );
...
test.describe('Responsiveness', () => {
  ...
  for (const vp of viewports) {
    test(`should be usable on ${vp.name} (${vp.width}x${vp.height})`, async ({
      page,
    }) => {
