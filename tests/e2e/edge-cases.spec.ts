...
test.describe('Application Edge Cases', () => {
  test('should handle invalid library name in URL', async ({ page }) => {
    await page.goto('http://localhost:8080/?benchmark=true&library=NonExistentLib&nodes=10');
  ...
  test('should handle non-numeric nodes in URL', async ({ page }) => {
    await page.goto('http://localhost:8080/?benchmark=true&library=Sigma.js&nodes=notanumber');
  ...
  test('should handle extremely large node counts gracefully', async ({ page }) => {
    ...
    await page.goto('http://localhost:8080/?benchmark=true&library=Sigma.js&nodes=100000', { timeout: 60000 });
  ...
  test('should maintain state when resizing window during active demo', async ({ page }) => {
    await page.goto('http://localhost:8080/?benchmark=true&library=Sigma.js&nodes=10');
    await expect(page.locator('.demo-container')).toBeVisible();
